...
#data are available in the "../input/" directorysudo yum install python3-tkinter
...
#Data preparation - fill in missing data
...
plt.show()
#factors affecting gdp of country
#cor_to_gdp=pd.Series()
#for col in data.columns.values[2:]:
    #if((col!='Total_GDP($)')&(col!='climate')&(col!='GDP($ per capita)')):
        #cor_to_gdp[col]=data['Total_GDP($)'].cor(data[col])
#abs_cor_to_gdp=cor_to_gdp.abs().sort_value(asending=False)
#cor_to_gdp=cor_to_gdp.loc[abs_cor_ti_gdp.index]
#print(cor_to_gdp)
